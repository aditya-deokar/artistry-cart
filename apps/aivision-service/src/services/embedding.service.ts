...
function getEmbeddingModel(): GoogleGenerativeAIEmbeddings {
    if (!embeddingModel) {
        ...
        logger.info('Initialized Google Gemini embeddings', { model: 'text-embedding-004' });
    }
...
/**
 * Generate image embedding using Google Gemini
 * Note: Gemini embeddings work best with text descriptions of images
 * For true image embeddings, consider generating a description first
 */
export async function generateImageEmbedding(imageUrl: string): Promise<EmbeddingResult> {
    try {
        ...
        // 1. Use Gemini Vision to generate an image description first
        // 2. Then embed that description
        // For now, we'll use the image URL as a proxy
        const text = `Image at URL: ${imageUrl}`;
